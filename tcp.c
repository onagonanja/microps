...
static char *tcp_flg_ntoa(uint8_t flg) {
  ...
  snprintf(str, sizeof(str), "--%c%c%c%c%c%c", TCP_FLG_ISSET(flg, TCP_FLG_URG) ? 'U' : '-', TCP_FLG_ISSET(flg, TCP_FLG_ACK) ? 'A' : '-', TCP_FLG_ISSET(flg, TCP_FLG_PSH) ? 'P' : '-', TCP_FLG_ISSET(flg, TCP_FLG_RST) ? 'R' : '-', TCP_FLG_ISSET(flg, TCP_FLG_SYN) ? 'S' : '-',
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface) {
  ...
  if(src == IP_ADDR_BROADCAST || dst == IP_ADDR_BROADCAST) {
    errorf("src or dst addr is braodcast");
    return -1;
  }
