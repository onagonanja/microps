...
static int arp_request(struct net_iface *iface, ip_addr_t tpa) {
  ...
  return net_device_output(iface->dev, ETHER_TYPE_ARP, (uint8_t *)&request, sizeof(request), &IP_ADDR_BROADCAST);
}
...
int arp_init(void) {
  struct timeval interval = {ARP_CACHE_TIMEOUT, 0};

  if(net_protocol_register(NET_PROTOCOL_TYPE_ARP, arp_input) == -1) {
    errorf("failed to register ARP");
    return -1;
  }
  if(net_timer_register(interval, arp_timer_handler) == -1) {
    errorf("failed to register arp_timer_handler");
    return -1;
  }
  return 0;
}