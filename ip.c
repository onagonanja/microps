...
static struct ip_route *ip_route_add(ip_addr_t network, ip_addr_t netmask, ip_addr_t nexthop, struct ip_iface *iface) {
  ...
  infof("route added: network=%s, netmask=%s, nexthop=%s, iface=%s dev=%s", ip_addr_ntop(route->network, addr1, sizeof(addr1)), ip_addr_ntop(route->netmask, addr2, sizeof(addr2)), ip_addr_ntop(route->nexthop, addr3, sizeof(addr3)), ip_addr_ntop(route->iface->unicast, addr4, sizeof(addr4)),
...
struct ip_iface *ip_iface_alloc(const char *unicast, const char *netmask) {
  ...
  if(ip_addr_pton(unicast, &(iface->unicast)) == -1) {
    errorf("ip_addr_pton(unicast, iface->unicast) failure");
    memory_free(iface);
    return NULL;
  };
...
struct ip_iface *ip_iface_select(ip_addr_t addr) {
  ...
  for(iface = ifaces; iface; iface = iface->next) {
    if(iface->unicast == addr || iface->broadcast == addr) {
      return iface;
    }
  }
  return NULL;
...
int ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)) {
  ...
  for(entry = protocols; entry; entry = entry->next) {
    if(entry->type == type) {
      errorf("protocol type already exists");
      return -1;
    }
  }
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  if(len < IP_HDR_SIZE_MIN) {
    errorf("too short: %d", len);
    return;
  }
  hdr = (struct ip_hdr *)data;
  v = (hdr->vhl & 0xf0) >> 4;
  if(v != IP_VERSION_IPV4) {
    errorf("ip version is not v4");
    return;
  }
  hlen = (hdr->vhl & 0x0f) << 2;
  if(hlen > len) {
  ...
  if((iface = net_device_get_iface(dev, NET_IFACE_FAMILY_IP)) == -1) {
    return;
  }
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst) {
  ...
  if(src == IP_ADDR_ANY && dst == IP_ADDR_BROADCAST) {
    errorf("source address is requred for broadcast addresses");
    return -1;
  }
  route = ip_route_lookup(dst);
  if(!route) {
